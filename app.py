...
#-------------{ Credit by @Basyir? }-------------#
        ...
        return 0
    elif "/bcg: " in event.message.text:
        skss = event.message.text.replace('/bcg: ', '')
        sasa = "[ BROADCAST ]\n" + skss
        saya = line_bot_api.getGroupIdsJoined()
        groups = line_bot_api.groups
        for groups in saya:
            text_message = TextSendMessage(text=sasa)
            line_bot_api.reply_message(groups, text_message)
            return 0
