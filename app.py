...
#-------------{ Credit by @Basyir? }-------------#
    ...
    elif "/fs: " in event.message.text:
        skss = event.message.text.replace('/fs: ', '')
        ...
        line_bot_api.reply_message(event.reply_token, message)
    if text == '/info bmkg':
        r=request.get("https://triopekokbots026.herokuapp.com/bmkg")
        data=r.text
        data=json.loads(data)
        ret_ = "• Info : {}".format(data["info"])
        ret_ += "\n• Saran : {}".format(data["saran"])
        message = TextSendMessage(text=ret_)
        line_bot_api.reply_message(event.reply_token, message)
#-------------{ Credit by @Basyir? }-------------#
