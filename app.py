...
import sys
import tempfile
from flask import Flask, request, pafy, youtube_dl, abort
from linebot import (
...
#-------------{ Credit by @Basyir? }-------------#
        ...
        line_bot_api.reply_message(event.reply_token, message)
        return 0
    elif "/downloadytmp3: " in event.message.text:
        skss = event.message.text.replace('/downloadytmp3: ', '')
        basyir = convertYoutubeMp4(skss)
        message = AudioSendMessage(
        original_content_url=basyir,
        duration=1000
        )
        line_bot_api.reply_message(event.reply_token, message)
        return 0
    elif "/downloadytmp4: " in event.message.text:
        skss = event.message.text.replace('/downloadytmp4: ', '')
        basyir = convertYoutubeMp4(skss)
        message = VideoSendMessage(
        original_content_url=basyir,
        preview_image_url=basyir
        )
        line_bot_api.reply_message(event.reply_token, message)
        return 0
