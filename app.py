...
import requests, json, pafy, youtube_dl, asyncio
...
#-------------{ Credit by @Basyir? }-------------#
    ...
    elif (text == 'Info bmkg') or (text == 'Bmkg'):
        r=request.get_data("https://triopekokbots026.herokuapp.com/bmkg")
        data=r.text
        data=json.loads(data)
        ret_ = "• Info : {}".format(str(data["info"]))
        ret_ += "\n• Saran : {}".format(str(data["saran"]))
        message = TextSendMessage(text=str(ret_))
        line_bot_api.reply_message(event.reply_token, message)
#-------------{ Credit by @Basyir? }-------------#
