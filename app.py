...
def handle_join(event):
    line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(text='Type /help for command :D\n\nFollow My Creator To Ask Questions ?\nhttps://line.me/ti/p/~savegenerasimicin')) 
...
#-------------{ Credit by @Basyir? }-------------#
    ...
    if text == '/cancelall':
        if isinstance(event.source, SourceGroup):
            group = line_bot_api.getGroup(event.source.sender_id)
            if group_invitee is None or group_invitee == []:
                line_bot_api.reply_message(
                    event.reply_token,
                    TextSendMessage(text="No Pending List"))
            else:
                invitee = [contact.mid for contact in group_invitee]
                for inv in invitee:
                    line_bot_api.cancel_group_invitation(event.source.group_id)
                line_bot_api.reply_message(
                    event.reply_token, TextSendMessage(text='i am come for clean pending invitelist'))
    ...
    if text == 'me':
         contact = line_bot_api.get_profile(event.source.user_id)
         dataProfile = [{
             "type": "bubble",
             "styles": {
             "header": {"backgroundColor": "#000000"},
             "hero": {"backgroundColor": "#808000"},
             ...
             "hero": {
             "type": "image",
             "url": "https://obs.line-scdn.net/"+ contact.picturePath,
             "size": "full",
             "aspectRatio": "1:1",
             "aspectMode": "fit",
             "action": {
             "type": "uri",
             "uri": "line://app/1636361179-N9dnRpYg?type=foimage&img=https://i.ibb.co/d65fvK6/1545047581225.jpg"}},
             "body": {
             "type": "box",
             "layout": "vertical",
             "contents": [{
             ...
             "contents": [{
                 "type": "text",
                 "text": "* Nama : \n* Status: "+ contact.displayName + contact.statusMessage,
                 "color": "#000000",
                 "wrap": True,
                 "size": "sm"}]}]},
                 "footer": {
                 ...
                 "contents": [{
                 "type": "icon",
                 "url": "https://i.ibb.co/Tr1mjYH/1545946474474.jpg",
                 "size": "xl"},{
                 "type": "text",
                 "text": "c??? ?????",
                 "align": "center",
                 ...
                 "action": {
                 "type": "uri",
                 "uri": "line://ti/p/~savegenerasimicin"}},{
                 "type": "spacer",
                 "size": "sm"}]}]}}]
         data = {
             "messages": [
...
#-------------{ Credit by @Basyir? }-------------#
        ...
        sasa = "line://app/1636361179-N9dnRpYg?type=fotext&text=http://line.me/R/ti/p/~" + skss
        ...
        line_bot_api.reply_message(event.reply_token, text_message)
        return 0
    elif "/ztext: " in event.message.text:
        skss = event.message.text.replace('/ztext: ', '')
        sasa = zalgo.zalgo().zalgofy("" + skss)
        text_message = TextSendMessage(text=sasa)
        line_bot_api.reply_message(event.reply_token, text_message)
        return 0
        ...
        url = requests.get("https://api.zicor.ooo/joox.php?song="+ skss)
        data = url.json()
        message = AudioSendMessage(
    ...
    if text == '/tiktok':
        ...
        line_bot_api.reply_message(event.reply_token, message)
    elif (text == 'Listtoken') or (text == '/listtoken'):
        message = TextSendMessage(text='List Token Bot\n• /chromeos\n• /win10\n• /iosipad\n• /desktopmac\n• /desktopwin\n• /clova\nNb: Type </done> For See Token')
        line_bot_api.reply_message(event.reply_token, message)
...
#-------------{ Credit by @Basyir? }-------------#
    ...
    elif (text == '/media') or (text == 'media') or (text == 'Help'):
        buttons_template = TemplateSendMessage(
    ...
    elif (text == '/media2') or (text == 'media2') or (text == 'Help'):
        buttons_template = TemplateSendMessage(
            alt_text='All About Trio Pekok Bots Media',
            template=ButtonsTemplate(
                thumbnail_image_url='https://i.ibb.co/Tr1mjYH/1545946474474.jpg',
...
#-------------{ Credit by @Basyir? }-------------#
        ...
        message = TemplateSendMessage(
            ...
            template=CarouselTemplate(
                columns=[
                    CarouselColumn(
                        thumbnail_image_url='https://media.giphy.com/media/Vt7tALoHXmcfe/giphy.gif',
                        title='TRIO PEKOK BOTS',
                        text='Free Bot Public',
                        align= 'center',
                        actions=[
                            URITemplateAction(
